...
export function SummaryList({ summaries, episodeId }: { summaries: Summary[]; episodeId: string }) {
  if (summaries.length === 0) {
    return (
      <Card className="p-8 text-center">
        <p className="text-muted-foreground">No summaries generated yet</p>
        <p className="mt-2 text-sm text-muted-foreground">Click "Generate Summaries" to create AI-powered content</p>
      </Card>
    )
  }
        ...
        <Card key={summary.id} className="p-6">
          <div className="flex items-start justify-between">
            <div className="flex-1">
              ...
              <p className="mt-3 text-sm leading-relaxed line-clamp-3">{summary.content}</p>
            ...
            <div className="ml-4 flex flex-col gap-2">
              ...
              <Button variant="outline" size="sm" onClick={() => navigator.clipboard.writeText(summary.content)}>
                Copy
              </Button>
            </div>
          </div>
