...
export function formatErrorForDisplay(error: unknown): FormattedError {
	...
	if (code === 503 || status === "UNAVAILABLE") {
		return {
			...
			message:
				"The AI model is currently experiencing high demand and is overloaded. This is a temporary issue.",
			action:
				"Please try again in a few moments. If the problem persists, try again in 5-10 minutes.",
			icon: "⏳",
		};
	}
